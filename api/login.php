<?php include_once($_SERVER['DOCUMENT_ROOT']."/app/php/common/include/session.php"   );

//echo "login\n";
//echo "userid:".$mid."\n";
//echo "profile:".$profile."\n";
//echo "fcmtoken:".$fcmtoken."\n";
//http://app2.intopet.co.kr/app/api/login.php?mid=%s&fcmtoken=%s&devicegbn=android
//."_".$_SERVER['REQUEST_URI']."_".$_SERVER['QUERY_STRING'] ))
$query = "INSERT INTO itapp_api_log SET
userid           = '".addslashes(iconv("utf-8", "euc-kr", $mid))."',
profile          = '',
...
if($mid=="" || $fcmtoken==""){
    echo "userid/fcmtoken no recv";
    echo "<br><br>연동 정보를 수신하지 못했습니다.<br>다른 로그인 방법을 이용해주세요";
    exit;
}
$userid = $mid;

$query_user_cnt = " SELECT count(*) cnt FROM itapp_user WHERE del_yn   = 'N' and user_id = '".$userid."'  "; //remark 08.07 11:15 and sign_type='intopet'
...
if($row_user_cnt[cnt] > 0){
    $query_upd = " UPDATE `intopet`.`itapp_user` set `push_token` = '".$fcmtoken."', device_gbn = '".$devicegbn."', login_status = 'Y'
                    WHERE `user_id` = '".$userid."'  "; //remark 20.08.07 11:15 and sign_type='intopet'
    ...
    $query_ins = " INSERT INTO `intopet`.`itapp_user` 
            	...
            	VALUES	('', '".$userid."', '".iconv("utf-8","euc-kr",$user_name)."', now(),          'system',  '".$_SERVER['REMOTE_ADDR']."','N',now(),       '".$devicegbn."',   '".$fcmtoken."','intopet') ";
    $result = mysql_query($query_ins);
//    echo "member info is not found.";
//    exit;
...
/*
로그인 화면에서 >
- 카카오 클릭시 : 클라이언트로 intopetapp://kakao_login? 호출
- 구글   클릭시 : 클라이언트로 intopetapp://google_login? 호출
- 아이디/패스워드로 로그인시  : 클라이언트로 intopetapp://login? 호출

클라이언트에서
- 카카오 로그인 성공시 : /app/api/kakao_login.php?
- 구글   로그인 성공시 : /app/api/google_login.php?
- 일반 아이디 (저장) 성공시 : /app/api/login.php?   
